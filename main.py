...
def normalized_metrics(y_true: pd.Series, y_pred: np.ndarray, y_scaler: MinMaxScaler):
...
def plot_regression_output_vs_target(
    ax,
    y_true,
    y_pred,
    title: str,
...
def save_scatter_plot(df: pd.DataFrame, features: list[str], dataset_name: str, out_dir: Path):
    ...
    fig.tight_layout()
    fig.savefig(out_dir / f"{dataset_name}_scatter_plot.png", dpi=150)
    plt.close(fig)


def plot_target_vs_all_models(
    y_true: pd.Series,
...
def save_model_regression_plots(
    ...
    fig.tight_layout()
    fig.savefig(out_dir / f"{dataset_name}_regression_{safe_slug(model_name)}.png", dpi=150)
    plt.close(fig)
...
def save_univariate_regression_plots(
    ...
    for feature in features:
        ...
        fig, axes = plt.subplots(1, 2, figsize=(10, 4.5))
        plot_regression_output_vs_target(
            axes[0], y_tr, uni_model.predict(X_tr), "Training", "C0", y_scaler
        )
        plot_regression_output_vs_target(
            axes[1], y_te, uni_model.predict(X_te), "Validation", "C2", y_scaler
        )
        fig.suptitle(f"MLP — input: {feature}", fontsize=10)
        fig.tight_layout()
        fig.savefig(
            out_dir / f"{dataset_name}_regression_feature_{safe_slug(feature)}.png",
            dpi=150,
        )
        plt.close(fig)
...
def train_one_dataset(dataset_path: Path, output_rows: list[dict]):
    ...
    save_scatter_plot(df, features, dataset_path.name)
    save_univariate_regression_plots(df, features, y, y_scaler, dataset_path.name)
    ...
    for name, model in models.items():
        ...
        mse, mae, rmse, r2 = normalized_metrics(y_test, y_pred, y_scaler)
        output_rows.append({
    ...
    plot_target_vs_all_models(
        ...
        model_predictions,
        y_scaler,
        PLOTS_DIR / f"{dataset_name}_test_output_all_models.png",
    )
    ...
    fig.tight_layout()
    fig.savefig(f"plots/{dataset_path.name}_regression_all_models_validation.png", dpi=150)
    plt.close(fig)
...
def main():
    ...
    results.to_csv("results_table/performance_table_all_datasets.csv", index=False)
